...
class NewsController extends Controller
{
    public function getGameNews(GameNewsMessagesRequest $request)
    {
        $newsCollection = News::where('message_type', '=', $request->messageType->value)
            ->where('max_player_level', '>=', $request->playerLevel)
        ...
        foreach ($newsCollection as $news) {
            ...
            $newsResponse->embeddedBackgroundImage = $news->background_image;
            $newsResponse->embeddedInGameNewsBackgroundImage = $news->in_game_news_background_image;
            $newsResponse->embeddedInGameNewsThumbnailImage = $news->in_game_news_thumbnail;
